...
C4030::C4030(std::string value) : AComponent(std::move(value))
{
    for (auto i : _pins) {
...
nts::Tristate C4030::compute(std::size_t pin)
{
    ...
    else if ((_pinsSt[_in[position][0]] == nts::FALSE && _pinsSt[_in[position][1]] == nts::TRUE) ||
             (_pinsSt[_in[position][0]] == nts::TRUE && _pinsSt[_in[position][1]] == nts::FALSE))
...
{
    std::cout << "Dump 4030" << std::endl;
}

std::string C4030::getType() const
{
    return (std::string("4030"));
}
...
std::vector<std::pair<nts::IComponent *, std::size_t>> C4030::getLinks(std::size_t pin)
{
